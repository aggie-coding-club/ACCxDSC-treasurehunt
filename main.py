    ...
    if request.method == 'POST':
        ...
        if 'acc' in json and 'dsc' in json:
            if json['acc'].lower() == "aggie coding club" and json['dsc'].lower() == "developer student club":
                return "That's Correct! For the next question, make a POST request to <url>/numofficers with a JSON object payload of key “num_officers” and the value as an integer number as your answer to how many officers acc has. Go to aggiecodingclub.com to find out!"
            else:
...
def initials(name):
    if request.method == 'GET':
        if name.lower() == 'dak':
            return "You're an expert! Next, make a POST request to <url>/next-dsc-event with the JSON object payload of key “word” and the value as the 2nd to last word of the event on October 10th.  Visit https://dsc.community.dev/texas-am-university for details"
        else:
